...
def upload_s3(domain, domain_file, domain_file_path, s3_folder):
    try:
        s3.upload_file(domain_file_path, s3_bucket_name, os.path.join(s3_folder, domain, domain_file))
    ...
    for domain in os.listdir(current_dir):
        ...
        for domain_file in domain_files:
            ...
            if domain_file.endswith(".txt"):
                ...
                create_metadata_file(response=response[f"data/{domain}/{example_name}"], metadata_filename=metadata_filename)
                upload_s3(domain, metadata_filename, os.path.join(current_dir, metadata_filename), 'ingest')
        ...
        for domain_file in domain_files:
            ...
            if not domain_file.endswith(".txt"):
                ...
                if domain_file.endswith(".yaml"):
                    response[f"data/{domain}/{example_name}"]["cfn_stack"] = f"s3://{s3_bucket_name}/data/{domain}/{domain_file}"
                    upload_s3(domain, domain_file, domain_file_path, 'data')
                elif domain_file.endswith('.jpeg') or domain_file.endswith('.png') or domain_file.endswith('.jpg'):
                    response[f"data/{domain}/{example_name}"]["architecture_image"] = f"s3://{s3_bucket_name}/data/{domain}/{domain_file}"
                    upload_s3(domain, domain_file, domain_file_path, 'data')
